...
Future<void> loadModels() async {
  try {
    plantDetectorInterpreter =
        await Interpreter.fromAsset('assets/plant_modelH5.tflite');
    healthCheckerInterpreter =
        await Interpreter.fromAsset('assets/health_modelH5.tflite');
...
Future<List<List<List<List<double>>>>> preprocessImage(
    File imageFile, List<int> inputShape) async {
  try {
...
Future<bool> isPlant(File imageFile) async {
  try {
    ...
    var inputShape = plantDetectorInterpreter
        .getInputTensor(0)
        .shape; // Example: [1, 180, 180, 3]
    ...
    var outputShape =
        plantDetectorInterpreter.getOutputTensor(0).shape; // Example: [1, 1]
    var output =
    ...
    double probability =
        output[0][0]; // Accessing the single value in the [1, 1] output

    // Interpret the result based on thresholding
    if (probability > 0.5) {
      print(
          'Image classified as Not Plant (Probability: ${probability.toStringAsFixed(2)})');
      return false;
    } else {
      print(
          'Image classified as Plant (Probability: ${(1 - probability).toStringAsFixed(2)})');
      return true;
    }
  } catch (e) {
...
Future<Map<String, dynamic>> checkPlantHealth(File imageFile) async {
  try {
    // Get input shape of the model
    var inputShape = healthCheckerInterpreter
        .getInputTensor(0)
        .shape; // Example: [1, 180, 180, 3]
    ...
    // Prepare the output tensor
    var outputShape =
        healthCheckerInterpreter.getOutputTensor(0).shape; // Example: [1, 1]
    var output =
        List.generate(outputShape[0], (_) => List.filled(outputShape[1], 0.0));
    ...
    if (probability > 0.5) {
      print(
          'Image classified as Unhealthy (Confidence: ${confidence.toStringAsFixed(2)})');
      return {'result': 'Unhealthy', 'confidence': confidence};
    } else {
      print(
          'Image classified as Healthy (Confidence: ${confidence.toStringAsFixed(2)})');
    ...
    return {
      'result': 'Unhealthy',
      'confidence': 0.0
    }; // Default to Unhealthy with 0 confidence on error
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> navigateToResultPage(BuildContext context, String result,
  ...
  Future<void> _handleImageSelection(ImageSource source) async {
    ...
    File? imageFile = await pickImageFromSource(source);
    if (imageFile != null) {
      ...
      if (isPlantImage) {
        ...
        await navigateToResultPage(context, result, confidence, imageFile.path);
      } else {
        showDialog(
          context: context,
                ...
                child: Column(
                  ...
                  children: [
                    ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors
                            .teal, // A modern and vibrant background color
                        foregroundColor:
                            Colors.white, // Text color for better contrast
                        padding: const EdgeInsets.symmetric(
                            vertical: 15.0,
                            horizontal: 30.0), // Spacious padding
                        ...
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(
                              25.0), // More rounded corners for a sleek look
                        ),
                        elevation: 5, // Adds a subtle shadow for depth
                        shadowColor: Colors
                            .black54, // Shadow color for better visibility
                      ),
                      child: const Row(
                        ...
                        children: [
                          Icon(Icons.camera_alt,
                              size: 22,
                              color: Colors
                                  .white), // Add an icon for better visual context
                    ...
                    ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            Colors.teal, // Eye-catching background color
                        foregroundColor:
                            Colors.white, // Text color for better contrast
                        padding: const EdgeInsets.symmetric(
                            vertical: 15.0,
                        ...
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(
                              25.0), // More rounded corners for a modern design
                        ),
                        elevation: 5, // Adds subtle depth with shadow
                        shadowColor:
                            Colors.black54, // Shadow color for visibility
                      ),
                      child: const Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Icon(Icons.image,
                              size: 22,
                              color: Colors.white), // Icon for context
                          SizedBox(width: 10), // Space between icon and text
                          Text(
                            'Gallery Image',
                            style: TextStyle(
